...
export async function GET(request: NextRequest) {
    ...
    if (!Moralis.Core.isStarted) {
        // Attempt to start Moralis if it hasn't been already.
        try {
            await Moralis.start({ apiKey: MORALIS_API_KEY });
        } catch (e) {
             console.error("Failed to start Moralis on-demand", e);
             return NextResponse.json({ error: 'Moralis SDK could not be initialized.' }, { status: 503 });
        }
    }
